...
/**
 * 应张剑要求，增加万能密码逻辑，方便运营人员接到用户反馈的问题的时候，可以直接通过用户的账号和万能密码模拟客户登陆，方便重现用户反馈的问题。这样做会存在密码安全性
 * 源码org.springframework.security.web.authentication.www.DigestAuthenticationFilter
 * 为了增加万能密码的逻辑，而在源码基础上，增加部分逻辑 见188-213行代码
 */
package com.common.security;
import com.common.utils.DigestAuthUtils;
 ...
 * agents (such as Internet Explorer and FireFox).
 * <p>
 ...
 * stored in the "nonce" that is sent to the client by the
 * {@link DigestAuthenticationEntryPoint}.
 * <p>
 ...
 * {@link DigestAuthenticationEntryPoint}, which will prompt the user to
...
//	private String universalPassword;
	...
	public void doFilter(ServletRequest req, ServletResponse res, FilterChain chain) throws IOException,
		...
		String header = StringEscapeUtils.unescapeHtml(request.getHeader("Authorization"));
		...
		// attempt
		System.out.println("digestAuth.getResponse()=" + digestAuth.getResponse());
		System.out.println("serverDigestMd5" + serverDigestMd5);
		if (!serverDigestMd5.equals(digestAuth.getResponse())) {

			// 如果配置了万能密码，则使用万能密码试一遍，开始
			// 应张剑要求，增加万能密码逻辑，方便运营人员接到用户反馈的问题的时候，可以直接通过用户的账号和万能密码模拟客户登陆，方便重现用户反馈的问题。这样做会存在密码安全性
//			if (universalPassword != null && !universalPassword.equals("")) {
//				String u_password = DigestAuthUtils.md5Hex(user.getUsername() + ":" + universalPassword);
//				serverDigestMd5 = digestAuth.calculateServerDigest(u_password, request.getMethod());
//
//				if (!serverDigestMd5.equals(digestAuth.getResponse())) {
//					if (logger.isDebugEnabled()) {
